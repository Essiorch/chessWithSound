...
function handleUpQueenWhite (event, whitePlace, blackPlace, coord, number, setColor, setCoord, setCount, arrayX, arrayY, color, gameOver, audio) {
	let arrayX1 = [...arrayX];
	let arrayY1 = [...arrayY];				
	...
	for (let i = 1; i < 8; i++) {
		if (whitePlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) + i] && item.Y == coord[number].Y && item.Flag == true).length != 0) break;
		diagonalLimit.push({X: arrayX[arrayX.indexOf(coord[number].X) + i], Y: coord[number].Y});
		if (blackPlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) + i] && item.Y == coord[number].Y).length != 0) break;
	}
	for (let i = 1; i < 8; i++) {
		if (whitePlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) - i] && item.Y == coord[number].Y && item.Flag == true).length != 0) break;
		diagonalLimit.push({X: arrayX[arrayX.indexOf(coord[number].X) - i], Y: coord[number].Y});
	...
	for (let i = 1; i < 8; i++) {
		if (whitePlace.filter(item => item.X == coord[number].X && item.Y == arrayY[arrayY.indexOf(coord[number].Y) - i] && item.Flag == true).length != 0) break;
		diagonalLimit.push({X: coord[number].X, Y: arrayY[arrayY.indexOf(coord[number].Y) - i]});
		if (blackPlace.filter(item => item.X == coord[number].X && item.Y == arrayY[arrayY.indexOf(coord[number].Y) - i]).length != 0) break;
	}
	for (let i = 1; i < 8; i++) {
		if (whitePlace.filter(item => item.X == coord[number].X && item.Y == arrayY[arrayY.indexOf(coord[number].Y) - i] && item.Flag == true).length != 0) break;
		diagonalLimit.push({X: coord[number].X, Y: arrayY[arrayY.indexOf(coord[number].Y) + i]});
...
export default handleUpQueenWhite;