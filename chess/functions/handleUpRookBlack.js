...
function handleUpRookBlack (event, whitePlace, blackPlace, coord, number, setColor, setCoord, setCount, arrayX, arrayY, color, gameOver, audio) {
	...
	let arr = XYLimits (blackPlace, whitePlace);
	
	if (x == coord[number].X && y != coord[number].Y && 
		y < arr.heightYLimit.Y && y > arr.downYLimit.Y && 
		y <= arr.heightYLimitOppositeColor.Y && y >= arr.downYLimitOppositeColor.Y && color === false && gameOver == false||
		x < arr.heightXLimit.X && x > arr.downXLimit.X && x <= arr.heightXLimitOppositeColor.X && 
		x >= arr.downXLimitOppositeColor.X && y == coord[number].Y && color === false && gameOver == false) {
			if (!(x == coord[number].X && y == coord[number].Y)) {
				setColor(true);
				audio.play();
			}
...
export default handleUpRookBlack;