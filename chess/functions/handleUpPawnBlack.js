...
function handleUpPawnBlack(event, whitePlace, blackPlace, coord, number, setColor, setCoord, setCount, arrayX, arrayY, color,  gameOver, audio) {
	...
	coord.filter(item => item.X == x && item.Y == y && item.Flag == true).length == 0 && color === false && gameOver == false) {
	...
	if ((whitePlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) - 1] && 
	...
	&& y == arrayY[arrayY.indexOf(coord[number].Y) + 1]) && color === false && gameOver == false) {
		let arr = coord;
...
export default handleUpPawnBlack;