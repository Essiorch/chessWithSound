...
function handleUpKingBlack (event, whitePlace, blackPlace, coord, number, setColor, setCoord, setCount, arrayX, arrayY, color,  gameOver, audio) {
	let arrayX1 = [...arrayX];
	let arrayY1 = [...arrayY];				
	...
	if (blackPlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) + 1] && item.Y == arrayY[arrayY.indexOf(coord[number].Y) + 1] && item.Flag == true).length == 0) {
	...
	if (blackPlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) + 1] && item.Y == coord[number].Y && item.Flag == true).length == 0) {
		diagonalLimit.push({X: arrayX[arrayX.indexOf(coord[number].X) + 1], Y: coord[number].Y});
	}
	if (blackPlace.filter(item => item.X == arrayX[arrayX.indexOf(coord[number].X) - 1] && item.Y == coord[number].Y && item.Flag == true).length == 0) {
		diagonalLimit.push({X: arrayX[arrayX.indexOf(coord[number].X) - 1], Y: coord[number].Y});
	}
	if (blackPlace.filter(item => item.X == coord[number].X && item.Y == arrayY[arrayY.indexOf(coord[number].Y) - 1] && item.Flag == true).length == 0) {
...
export default handleUpKingBlack;