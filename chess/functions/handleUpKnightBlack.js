...
function handleUpKnightBlack(event, whitePlace, blackPlace, coord, number, setColor, setCoord, setCount, arrayX, arrayY, color,  gameOver, audio) {
	...
	if (blackPlace.filter(item => item.X == x && item.Y == y && item.Flag == true).length == 0 && color === false && gameOver == false) {
...
export default handleUpKnightBlack;